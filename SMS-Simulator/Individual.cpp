...
akml::Matrix<float, GRAPH_SIZE-1, 1> Individual::computeUtilityGrad(std::array<SexualMarket::Link*, GRAPH_SIZE-1>* relations, std::tuple<akml::Matrix<float, P_DIMENSION, GRAPH_SIZE-1>,akml::Matrix<float, GRAPH_SIZE-1, 1>, akml::Matrix<Individual*, GRAPH_SIZE-1, 1>>* PS_Alpha) {
    ...
    for (unsigned short int line(1); line <= GRAPH_SIZE-1; line++){
        scalaralpha += (std::get<1>(*PS_Alpha)(line, 1) != 0) ? std::pow(std::get<1>(*PS_Alpha)(line, 1),this->gamma)/P_DIMENSION : 0;
    }
    
    std::get<1>(*PS_Alpha).transform([this, &scalaralpha](float val) { return (val != 0) ? (-2)*Individual::gamma*std::pow(val, Individual::gamma-1)*scalaralpha : 0; });
    grad = P_prod + std::get<1>(*PS_Alpha);
    //akml::cout_matrix(P_prod);
...
void Individual::takeAction(){
    ...
    std::tuple<akml::Matrix<float, P_DIMENSION, GRAPH_SIZE-1>,akml::Matrix<float, GRAPH_SIZE-1, 1>, akml::Matrix<Individual*, GRAPH_SIZE-1, 1>> PS_Alpha = this->buildPSAndAlpha(std::ref(relations));
