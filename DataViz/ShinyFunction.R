...
generate_plot <- function(gamma, delta, g1, g2) {
  G <- c(g1,g2)
  a <- seq(0,0.9,0.025)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("gamma", "Choose gamma:", min = 0, max = 15, value = 2, step = 0.2),
...
server <- function(input, output) {
  output$plot <- renderPlot({
    generate_plot(input$gamma, input$delta, input$g1, input$g2)
  })
}
